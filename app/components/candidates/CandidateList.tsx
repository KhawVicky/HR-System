...
const normalizeWeightsTo100 = <T extends { weight: number }>(
  ...
  const normalized = items.map((item) => ({
    ...item,
    weight: Math.floor((item.weight / totalOriginalWeight) * 100),
  }));

  let assignedTotal = normalized.reduce((sum, item) => sum + item.weight, 0);
...
const calculateScoreBreakdown = (scoreBreakdown: ScoreBreakdownItem[]) => {
  return normalizeWeightsTo100(scoreBreakdown).map((item) => ({
    ...item,
    weightedScore: Number(((item.criteriaScore / 100) * item.weight).toFixed(1)),
    maxWeightedScore: item.weight,
  }));
};

const calculateTotalScore = (scoreBreakdown: ScoreBreakdownItem[]) => {
  return Number(
    scoreBreakdown
...
const recalculateRanks = (candidateList: Candidate[]) => {
  ...
  const sortedRankable = [...candidateList]
    .filter((candidate) => rankableStatuses.includes(candidate.status))
    .sort((a, b) => {
